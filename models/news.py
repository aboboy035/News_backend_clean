...
class Base(DeclarativeBase):
     created_at:Mapped[datetime] = mapped_column(DateTime, default=datetime.now,insert_default=func.now(),comment="创建时间")
     updated_at:Mapped[datetime] = mapped_column(DateTime, default=datetime.now,onupdate=func.now(),comment="更新时间")
...
class Category(Base):
     ...
     sort_order: Mapped[int] = mapped_column(Integer,default=0,comment="排序",nullable=False)
     def __repr__(self):
